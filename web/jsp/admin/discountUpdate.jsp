...
<html>
    <head>
        ...
        <title>NestMart - Discount Update</title>
                ...
                <main class="content">
                    <div class="container mt-4">
                        <table class="table table-hover my-0" id="discountTable">
                            ...
                            <form action="${pageContext.request.contextPath}/admin/discountUpdate.htm" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="discountID" value="${discount.discountID}" />

                                <div class="form-group">
                                    <label for="discountName">Discount Name:</label>
                                    <input type="text" class="form-control" id="discountName" name="discountName" value="${discount.discountName}" required>
                                </div>


                                <div class="form-group">
                                    <label for="description">Description:</label>
                                    <textarea class="form-control" id="description" name="description" rows="4" required>${discount.description}</textarea>
                                </div>

                                <div class="form-group">
                                    <label for="startDate">Start Date:</label>
                                    <input type="datetime-local" class="form-control" id="startDate" name="startDate" value="${discount.formattedStartDate}" required>
                                </div>

                                <div class="form-group">
                                    <label for="endDate">End Date:</label>
                                    <input type="datetime-local" class="form-control" id="endDate" name="endDate" value="${discount.formattedEndDate}" required>

                                </div>

                                <div class="form-group">
                                    <label for="image">Current Image:</label>
                                    <c:if test="${not empty discount.image}">
                                        <img src="${pageContext.request.contextPath}/assets/admin/images/uploads/discount/${discount.image}" alt="Current Image" class="img-thumbnail mb-2" style="max-width: 200px;">
                                    </c:if>
                                    <input type="file" class="form-control" id="imageFile" name="imageFile" />
                                </div>

                                <button type="submit" id="updateDiscountButton" class="btn btn-primary">Update Discount</button>
                                <a href="${pageContext.request.contextPath}/admin/discount.htm" class="btn btn-secondary">Cancel</a>
