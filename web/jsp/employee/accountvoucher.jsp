                ...
                <main class="content">
                    <div class="container mt-4">
                        <div class="search-container">
                            <form action="${pageContext.request.contextPath}/employee/accountVouchers.htm" method="get" class="search-form">
                                <input type="text" name="keyword" class="form-control search-input"
                                       placeholder="Search by Account ID, Full Name, Voucher ID, Voucher Code,..." value="${keyword}">
                            ...
                            <div class="icon-container">
                                <button class="btn btn-add"
                                        onclick="window.location.href = '${pageContext.request.contextPath}/employee/accountVoucherCreate.htm'">
                        ...
                        <c:if test="${not empty keyword}">
                            <c:choose>
                                <c:when test="${not empty listAccountVoucher}">
                                    <p>Found <b>${totalAccountVouchers}</b> results.</p>
                                </c:when>
                                <c:otherwise>
                        ...
                        <table class="table table-hover my-0" id="accountVoucherTable">
                            <thead>
                                <tr>
                                    <th>AccountVoucher ID</th>
                                    <th>Account ID</th>
                                    <th>Full Name</th>
                                    <th>Voucher ID</th>
                                    <th>Voucher Code</th>
                            ...
                            <tbody>
                                <c:forEach var="av" items="${listAccountVoucher}">
                                    <tr>
                                        <td>${av.accountVoucherID}</td>
                                        <td>${av.accountID}</td>
                                        <td>${av.fullName}</td>
                                        <td>${av.voucherID}</td>
                                        <td>${av.voucherCode}</td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${av.used}">Used</c:when>
                                                <c:otherwise>Not Used</c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>${av.formattedAssignedDate}</td>
                                        <td>${av.formattedExpiryDate}</td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${av.orderID != null}">${av.orderID}</c:when>
                                                <c:otherwise>-</c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td class="table-actions">
                                            <div class="btn-container">
                                                <a href="${pageContext.request.contextPath}/employee/accountVoucherUpdate.htm?accountVoucherID=${av.accountVoucherID}"
                                                   class="btn btn-update" title="Edit">
                                                    <i data-feather="edit"></i>
                                                </a>
                                                <a href="${pageContext.request.contextPath}/employee/accountVoucherDelete.htm?accountVoucherID=${av.accountVoucherID}"
                                                   class="btn btn-delete" title="Delete"
                                                   onclick="return confirm('Are you sure you want to delete this account voucher?');">
